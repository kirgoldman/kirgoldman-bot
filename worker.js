...
<blockquote>❗️ ВАЖНО: это не теория — за методом стоят более 30 проектов в разных нишах с окупаемостью от 40% до ~400%.</blockquote>
...
async function sendMsg1Reminder(env, chatId, text, image, buttonText) {
    await tg(env, "sendPhoto", {
          chat_id: chatId,
          photo: image,
          caption: text,
          parse_mode: "HTML",
          reply_markup: kb([{ text: buttonText, url: goUrl(env, "case", chatId) }]),
    });
}
...
async function runDueEvents(env) {
  ...
  for (const ev of results) {
        try {
                switch (ev.type) {
                  case "msg1_r1":
                              await sendMsg1Reminder(env, ev.user_id, MSG1_R1_TEXT, IMG.hookR1, "📖 Забрать разбор");
                              break;
                  case "msg1_r2":
                              await sendMsg1Reminder(env, ev.user_id, MSG1_R2_TEXT, IMG.hookR2, "👀 Посмотреть кейс");
                              break;
                  case "msg3":
